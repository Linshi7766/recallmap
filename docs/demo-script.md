...
Target length: 3:00. Spoken narration is 292 words; bracketed cues are visual directions, not narration.
...
### 1:44–2:07 — Repair in the student's words
...
### 2:07–2:25 — Make the change visible

[Select **Check my repaired understanding**. Show the green repaired node and Before/After card.]

“The priority node changes from misconception to supported, and the Before/After view shows the conceptual change. Recall verifies repaired reasoning; it does not merely supply an answer.”

### 2:25–2:50 — Explain the technology and disclosure

[Show the result, then briefly show the repository test/docs view.]

“At runtime, GPT-5.6 uses the Responses API with structured outputs. Codex helped research, build, and test Recall; it is not the runtime tutor. The exact built-in fallback runs only after an eligible live-model failure. Pasted material never uses fallback.”
