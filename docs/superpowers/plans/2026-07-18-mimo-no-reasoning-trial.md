...
- Archive integrity: the tracked-only `recall-mimo-none-20260718.tar.gz` had SHA-256 `6bccbaa61176a34d6f37c52841f66a06c3cdb4f772a3bf5454959237761f56a4`. Its exclusion scan found zero `node_modules`, `.next`, `.env`, or `.git` entries, and the uploaded archive hash matched the local hash.
...
- Exactly one paid controlled flow was run with novel, non-sensitive input. Generate returned status `200`, `fallback=false` in 4.157 s. Diagnose returned status `200`, `fallback=false` in 11.803 s with `3` nodes and a present probe. Classification: **healthy**.
- Product acceptance: **passed**. The diagnosis satisfied `200`, non-fallback, `nodes>=3`, probe present, and elapsed time at most 60 seconds. No second paid flow, browser flow, retry, or Responses parameter adjustment was performed; browser success was not observed or inferred.
...
- Local archive cleanup: the exact absolute archive path was resolved inside the designated worktree. Per Step 8, it remains present until this evidence commit completes and will then be the only file deleted; no remote archive or backup is authorized for deletion.
