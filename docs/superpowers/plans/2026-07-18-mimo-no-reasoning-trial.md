...
## 2026-07-18 MiMo no-reasoning deployment evidence

- Release commit: `2fde6789de12b894178de054af230e7113708e95`.
- Archive integrity: immediately before archive creation, `git status --short` produced no entries. The tracked-only `recall-mimo-none-20260718.tar.gz` had SHA-256 `6bccbaa61176a34d6f37c52841f66a06c3cdb4f772a3bf5454959237761f56a4`. Its exclusion scan found zero `node_modules`, `.next`, `.env`, or `.git` entries, and the uploaded archive hash matched the local hash.
- Read-only preflight: `/` and `/home` had 53 GiB free. Memory reported 285 MiB available with 1.3 GiB swap free. `recall.service` was active and enabled. `/home/azureuser/recall` was `azureuser:azureuser` mode `700`; `.env.production` was `azureuser:azureuser` mode `600`. Only the environment variable name `MIMO_API_KEY` was observed. Both required old backups existed, while the new backup and staging paths were absent. The Recall Nginx configuration showed `proxy_read_timeout 120s` in the server block for `recall-app.duckdns.org`; no Nginx change was made.
- Staging: the uploaded hash was rechecked before extraction. The copied `.env.production` retained `azureuser:azureuser` mode `600`, and its variable-name set matched the live file. `npm ci` exited `0`, reporting the two known moderate audit findings without remediation. `npm run build` exited `0`, with `/` and `/api/learn` remaining dynamic.
- Switch and health: the staged release replaced `/home/azureuser/recall` successfully; readiness returned local HTTP `200` on attempt 2. Rollback did not occur. The final service was active and enabled, local and public HTTP returned `200`, and the public provider label was `MiMo V2.5`. The old process recorded SIGTERM exit `143` before the new `Started` event; the new process was ready in 155 ms, and the post-start log window contained no startup-error or credential markers.
- Exactly one paid controlled HTTP production flow was run with novel, non-sensitive input. Generate returned status `200`, `fallback=false` in 4.157 s. Diagnose returned status `200`, `fallback=false` in 11.803 s with `3` nodes and a present probe. Classification: **healthy**.
- Controlled HTTP production acceptance: **passed**. The diagnosis satisfied `200`, non-fallback, `nodes>=3`, probe present, and elapsed time at most 60 seconds. The browser UI was not run, browser success was not observed, and browser success is not inferred. No second paid flow, retry, or Responses parameter adjustment was performed.
- Retained backups: `/home/azureuser/recall-backup-before-timeout-20260718`, `/home/azureuser/recall-backup-before-budget-20260718`, and `/home/azureuser/recall-backup-before-none-20260718`, all observed as `azureuser:azureuser` mode `700`. The uploaded archive was also retained.
- Local archive cleanup: the exact path `D:\UniFiles\ProjectRecall\.worktrees\mimo-timeout-retry\recall-mimo-none-20260718.tar.gz` was resolved and verified to be inside the designated worktree. Only that local file was deleted, and a subsequent existence check confirmed it no longer existed. The uploaded remote archive and all three named remote backups were retained.
