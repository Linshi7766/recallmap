...
export const repairInstructions = [
  ...
  "For a diagnosis with a priority node: priority status correct means overallStatus repaired; misconception changing to incomplete means overallStatus partial; otherwise use overallStatus not_repaired.",
  "For a transfer diagnosis with no priority node: all nodes correct means repaired, some nodes correct means partial, and none correct means not_repaired.",
  "Whenever any node status changes, both its claim and diagnosis must change to describe the current reasoning.",
