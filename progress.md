...
- **Status:** historical deployment and health-check record; its diagnosis acceptance result is **superseded** by the final MiMo no-reasoning trial below.
...
- **Status:** controlled HTTP acceptance passed on release `2fde678`; the current MiMo acceptance tracker is not blocked.
- Controlled `generate`: HTTP 200 in 4.157 seconds. Controlled `diagnose`: HTTP 200 in 11.803 seconds, `fallback=false`, three valid reasoning nodes, and a probe.
- The preserved 503/119.9-second Task 4 diagnose result above is superseded historical evidence, not the current acceptance state.
- Browser UI repair-flow verification remains unverified; no browser UI acceptance is claimed.
